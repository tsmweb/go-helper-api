...
// OutboxEvent represents a message that contains the event to be sent or received (outbox pattern).
type OutboxEvent struct {
	// ID unique id of each event.
	ID string `json:"id"`
...
type ExportedEvent interface {
	...
	// line cancelation event. This id will be used as the key for KafkaWrap messages later on. That way, all events
	// pertaining to one aggregate root or any of its contained sub-entities will go into the same partition of that
	// KafkaWrap topic, which ensures that consumers of that topic will consume all the events related to one and the same
	...
	//
	// This value will be used to route events to corresponding topics in KafkaWrap, so there’d be a topic for all events
	...
	// of its aggregate root, "order", ensuring that also this event will go into the "order" KafkaWrap topic.
...
// BuildOutboxEvent takes ExportedEvent as a parameter and returns an OutboxEvent.
func BuildOutboxEvent(event ExportedEvent) *OutboxEvent {
	return &OutboxEvent{
		ID:            uuid.New().String(),
